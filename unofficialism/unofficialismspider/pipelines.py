...
class UnofficialismspiderPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        document_definition = { 'title':item[ 'title'],
        ...
        documents = list(self.client.QueryDocuments(
            self.feedcoll['_self'],
            {
                ...
                'parameters': [
                    { 'name':'@url', 'value':document_definition['url'] }
                ]
            }))
        if (len(documents) < 1):
            # only create if it's fully new document
            print("document is added:title:%s" % item['title'] )
